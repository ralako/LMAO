...
def simulacia(r_index, mC_index):
    ...
    oblak.spracovanie()
    return (r_index, mC_index, oblak.rhos_stred_vysl, oblak.sigmy_vysl, oblak.zac_fitu_A_sigma_cas)
...
def main():
    ...
    vysledky_zac_fitu_A_sigma_cas = [[0 for i in range(mC_num)] for j in range(r_num)]
    ...
    with ProcessPoolExecutor() as executor:
        for result in executor.map(simulacia, r_index_pole, mC_index_pole):
            ...
            r_index, mC_index, rhos_stred_vysl, sigmy_vysl, zac_fitu_A_sigma_cas = result
            ...
            vysledky_zac_fitu_A_sigma_cas[r_index][mC_index] = zac_fitu_A_sigma_cas
    ...
    vysledky_nazvy = ["vysledky_rhos_"+rozmery+"_"+str(T)+"K.npy",
                      ...
                      "vysledky_zac_fitu_A_sigma_cas_"+rozmery+"_"+str(T)+"K.npy",
                      "config_"+rozmery+"_"+str(T)+"K.npy"]
    
    vysledky = [vysledky_rhos,vysledky_sigmy,vysledky_rhos_po10s,vysledky_sigmy_po10s,
                vysledky_zac_fitu_A_sigma_cas,np.array([r_moznosti,mC_moznosti],dtype=object) ]
